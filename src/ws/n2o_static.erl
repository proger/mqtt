-module(n2o_static).
-description('N2O Cowboy Static Backend').
...
rest_init(Req, {dir, Path, Extra}) ->
	...
	n2o:info(?MODULE,"Rest Init: ~p~n\r",[FileName]),
...
content_types_provided(Req, State={Path, _, Extra}) ->
    n2o:info(?MODULE,"Content Type Provided: ~p~n\r",[Path]),
...
get_file(Req, State={Path, {ok, #file_info{size=_Size}}, _}) ->
    ...
    n2o:info(?MODULE,"Split Path: ~p~n\r",[SplitPath]),
    %n2o:info(?MODULE,"Code Path: ~p~n\r",[filename:join([code:lib_dir(Name)|RestPath])]),
	FileName = filename:absname(StringPath),
    n2o:info(?MODULE,"Abs Name: ~p~n\r",[FileName]),
    ...
    {error,_} ->
        ...
        {error,_} ->
            ...
            {error,_} -> <<>> end end end,
    n2o:info(?MODULE,"Cowboy Requested Static File: ~p~n\r",[filename:absname(StringPath)]),
